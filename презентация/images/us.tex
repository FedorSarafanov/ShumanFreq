...
\begin{tikzpicture}[scale=1.5]
 ...
 %    }
   ...
   \draw[fill=black!5] (-0.25,0) node[yshift=1em, xshift=1em] {Ю} rectangle (0.25,4) node[xshift=-1em,yshift=-1em] {С};

    % \draw[xshift=2cm, fill=black!10] (0,0) arc (0:180:2) -- cycle; 

    % % \draw[xshift=4cm, dashed, opacity=0.7] (0,0) arc (0:180:4); 

    \foreach \i in {1,1.04,...,3} {
        \draw[] (-0.25,\i) -- ++(0.5,0);
    }
    \begin{scope}[rotate=90, xshift=2cm, yshift=-2cm]
    \draw[fill=black!5] (-0.25,0) node[left, yshift=1em] {В} rectangle (0.25,4) node[right, yshift=-1em] {З};
      \foreach \i in {1,1.04,...,3} {
        \draw[] (-0.25,\i) -- ++(0.5,0);
    }      
    ...
    \draw[] (0,2) node[align=center] {$\bigotimes$};
